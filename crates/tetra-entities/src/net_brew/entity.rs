...
struct UlForwardedCall {
    ...
    kind: UlForwardKind,
    /// Carrier carrying this forwarded call
    carrier_num: u16,
}
...
pub struct BrewEntity {
    ...
    /// UL calls being forwarded to TetraPack, keyed by carrier/timeslot
...
impl BrewEntity {
    ...
    fn handle_group_call_end(&mut self, queue: &mut MessageQueue, uuid: Uuid, _cause: u8) {
        ...
        if let Some(jitter) = self.dl_jitter.remove(&uuid) {
            if let (Some(carrier_num), Some(ts)) = (call.carrier_num, call.ts) {
                if !jitter.is_empty() {
                    tracing::debug!(
                        "[{}] BrewEntity: GROUP_IDLE uuid={} moving {} buffered frames to drain carrier={} ts={}",
                        self.log_label(),
                        uuid,
                        jitter.len(),
                        carrier_num,
    ...
    fn drain_jitter_playout(&mut self, queue: &mut MessageQueue) {
        ...
        for (uuid, call) in &self.active_calls {
            ...
            let Some(carrier_num) = call.carrier_num else {
                continue;
            };
    ...
    fn rx_network_call_ready(&mut self, queue: &mut MessageQueue, brew_uuid: Uuid, call_id: u16, carrier_num: u16, ts: u8, usage: u8) {
        ...
        if let Some(call) = self.active_calls.get_mut(&brew_uuid) {
            ...
            tracing::warn!(
                "[{}] BrewEntity: NetworkCallReady for unknown uuid={} (call already gone) — releasing orphaned circuit call_id={} carrier={} ts={}",
                self.log_label(),
                brew_uuid,
                call_id,
                carrier_num,
