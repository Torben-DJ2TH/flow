        ...
        for rx_slot in rx {
            if let Some(rx_slot) = rx_slot {
                ...
                if rx_slot.slot.train_type != TrainingSequence::NotFound {
                    tracing::trace!(ts=%self.dltime, carrier=rx_slot.carrier_num, "rx_tpsap_prim got {:?} in fullslot", rx_slot.slot.train_type);
                ...
                if rx_slot.subslot1.train_type != TrainingSequence::NotFound {
                    tracing::trace!(ts=%self.dltime, carrier=rx_slot.carrier_num, "rx_tpsap_prim got {:?} in subslot1", rx_slot.subslot1.train_type);
                    if slot_sent {
                ...
                if rx_slot.subslot2.train_type != TrainingSequence::NotFound {
                    tracing::trace!(ts=%self.dltime, carrier=rx_slot.carrier_num, "rx_tpsap_prim got {:?} in subslot2", rx_slot.subslot2.train_type);
                    if slot_sent {
                        tracing::debug!("Multiple candidate bursts detected in one slot; forwarding all (LMAC CRC drops false positives)");
                    }
                    if let Some(ul_rx_sender) = &self.ul_rx_sender {
